...
def make_receipt_fallback_capsule(encoded: str):
    r = receipt()
...
def test_single_round_receipt_outputs_are_replay_fallback():
    ...
    assert scenario.leader_results == ["0x01", "0x02"]
    assert "receipt.eqBlocksOutputs" in scenario.notes[0]


def test_padding_only_receipt_is_not_fake_replay_evidence():
    with pytest.raises(ReplayError, match="requires leader_results"):
        scenario_from_capsule(make_receipt_fallback_capsule("c786706164646564"))


def test_multi_round_receipt_outputs_are_not_misattributed():
    r = receipt(rounds=2)
    r["eqBlocksOutputs"] = "c9010286706164646564"
    cap = Capsule.build(
        tool_version="0.2.0",
        captured_at="2026-09-06T00:00:00Z",
        tx_id=TX_ID,
        capture_level="protocol",
        network={"rpc_url": "http://localhost:4000/api"},
        files={
            "transaction/receipt.json": canonical_json_bytes(r),
            "analysis/summary.json": b"{}\n",
            "capture/issues.json": b"[]\n",
    ...
    with pytest.raises(ReplayError, match="not round-keyed"):
